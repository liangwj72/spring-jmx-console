...
import com.liangwj.spring.jmxConsole.utils.MethodUtil.MethodInfoOfSetter;
...
public class MBeanUtils {
	...
	public static ClassLoadingInfo getClassLoadingInfo() {
		final ClassLoadingInfo bean = new ClassLoadingInfo();
	...
	public static ThreadingInfo getThreadingInfo() {
		final ThreadingInfo bean = new ThreadingInfo();
	...
	public static VmFullInfo getVmFullInfo() {
		final VmFullInfo bean = new VmFullInfo();
		updateMBeanAttrInfoToBean("java.lang:type=Runtime", bean);
		return bean;
	}
	...
	public static Map<String, Object> objectToMap(Object obj) throws Exception {
		...
		for (final PropertyDescriptor property : propertyDescriptors) {
			...
			if (getter != null) {
				...
				final IgnoreWhenObjectToMap anno = getter.getAnnotation(IgnoreWhenObjectToMap.class);
				if (anno != null) {
	...
	private static void updateMBeanAttrInfoToBean(String objectNameStr, Object bean) {
		...
		final Map<String, MethodInfoOfSetter> setterMap = new HashMap<>();
		final List<MethodInfoOfSetter> setters = MethodUtil.findSetter(bean.getClass());
		for (final MethodInfoOfSetter setter : setters) {
		...
		try {
			...
			if (!objectName.isPattern() && server.isRegistered(objectName)) {
				...
				for (final MBeanAttributeInfo attr : info.getAttributes()) {
					// 获取收字母小写的名字
					final String key = StringUtils.uncapitalize(attr.getName());

					// System.out.println(String.format("private %s %s;",
					// attr.getType(), attr.getName()));

					final MethodInfoOfSetter setter = setterMap.get(key);
					if (setter != null) {
			...
			// 如果出错就啥都不做
			log.error("获取MBean信息时，出现了错误, objectName=" + objectNameStr, e);
		}
	}
