...
public class ClassUtil {
	 ...
	 */
	public static long getClassBuildTime(String className) throws IOException {
		...
		if (url != null) {
			...
			if ("file".equals(protocol)) {
				...
				if (log.isDebugEnabled()) {
					log.debug("{}存在文件系统中，文件的最后更改时间是{}", className, new Date(time));
				}
				return time;
				...
				if (log.isDebugEnabled()) {
					log.debug("{}存在jar文件{}中，jar文件的最后更改时间是{}", className, jarFileName, new Date(time));
				}
				return time;
			} else {
